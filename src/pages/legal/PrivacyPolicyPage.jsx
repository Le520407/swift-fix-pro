...
const PrivacyPolicyPage = () => {
  ...
  useEffect(() => {
    const loadPrivacy = async () => {
      try {
        setLoading(true);
        const privacyDocument = await loadLegalDocument('privacy-policy');
        setDocument(privacyDocument);
        if (privacyDocument.sections.length > 0) {
          setActiveSection(privacyDocument.sections[0].id);
        }
      } catch (error) {
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600">Unable to load Privacy Policy</p>
        </div>
      </div>
    );
  }
